...
# #
# #         countDistinct = columnDistinctValues[column]
# #         features[column]['numOfDistinctValues'] = countDistinct + features[column]['numOfDistinctValues']
#
#
#
...
def clacForFile(features, file):
    ...
    #     res = spark.sql('select count(*) as xx from (select sum(clicked) as sCli, {0} from countClicks  where {0} is not null group by {0}) where sCli > 0'.format(column))\
    #         .collect()
    #
    #     res2 = spark.sql(
    #         'select count(*) as xx from (select sum(clicked) as sCli, {0} from countClicks  where {0} is not null group by {0}) where sCli = 0'.format(
    #             column)) \
    #         .collect()
    #
    #     val1 = res[0]['xx']
    #     val2 = res2[0]['xx']
    #
    #     print('res -> res -> res -> res -> res -> res -> res -> {0} - {1} {2} -> {3}'.format(column, val1, val2, val1 /(val1 + val2)))
    #
    #
    # return None
...
allFiles = ['/home/gabib3b/mycode/git/optimalQTest/data/batch_withIndex_1.csv'
...
for fileIndex in range(1, 30):
    ...
    for column in ALL_COLUMNS:
        ...
        #features[column]['distinctPerFile']

    print('running on index {0}'.format(fileIndex))
    file1 =  '/home/gabib3b/mycode/git/optimalQTest/data/batch_withIndex_{0}.csv'.format(fileIndex)
    file2 = '/home/gabib3b/mycode/git/optimalQTest/data/batch_withIndex_{0}.csv'.format(fileIndex + 1)
    file3 = '/home/gabib3b/mycode/git/optimalQTest/data/batch_withIndex_{0}.csv'.format(fileIndex + 2)

    clacForFile(features, [file3])
